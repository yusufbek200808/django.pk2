...
from crud.forms import CommentForm
...
def like_article(request,slug):
    ...
    if request.user not in article.likes.all():
        ...
        article.dislakes.remove(request.user)
...
def dislike_article(request,slug):
    ...
    if request.user not in article.dislakes.all():
        article.dislakes.add(request.user)
        article.likes.remove(request.user)
    elif request.user in article.dislakes.all():
        article.dislakes.remove(request.user)
    return redirect('article_detail',slug=slug)
